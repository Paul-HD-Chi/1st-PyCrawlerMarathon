...
class Day029ProjectPipeline(object):
...
class JSONPipeline(object):
    ...
    def close_spider(self, spider):
        ...
        print(f"Paul-store_file_path = {self.store_file_path}")
        # 以爬蟲的 board name + 日期當作存檔檔名
        print(f"Paul-spider.name = {spider.name}")
        print(f"Paul-board = {spider.board}")
        
        if spider.name == 'Day029Crawler' and spider.board:
        ...
        print(f"Paul-run-time-path = {self.runtime_file_path}")
        print(f"Paul-path = {self.store_file_path}\n")
